...
void mttkrp_map_DT(map<string, Tensor<>> &mttkrp_map,
  ...
  int K = W[0].ncol;
  if (mttkrp_map.find(args) != mttkrp_map.end())
    return;
  char seq_w[3];
  seq_w[2] = '\0';
  seq_w[1] = '*';
  if (args.length() == V.order / 2 || args.length() == V.order / 2 + 1) {
    ...
    char_string_copy(seq_f, index_start, parent[args], index_start + 1,
                     V.order - index_start - 1);
    seq_f[V.order - 1] = '*';
    ...
    for (int j = 1; j < sibling[args].length(); j++) { // iterate on [ab]
      ...
      char_string_copy(seq_f, 0, parent[args], 0, index_start);
      char_string_copy(seq_f, index_start, parent[args], index_start + j + 1,
                       V.order - index_start - j - 1);
      // build len for V_temp
  ...
  for (int j = 0; j < sibling[args].length(); j++) { // iterate on [ab]
    ...
    char_string_copy(seq_f, 0, parent[args], 0, index_start);
    char_string_copy(seq_f, index_start, parent[args], index_start + j + 1,
                     V_parent.order - index_start - j - 2);
    // build len for V_temp
...
void Construct_Dimension_Tree(map<string, string> &parent,
                              map<string, string> &sibling, int start,
                              int end) {
  if (end == start)
    return;
  if (end == start + 1) {
...
void build_V_vec(Tensor<> &V, Vector<> *W, int order, World &dw) {
  char chars[] = {'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p', 'q', 'r',
                  's', 't', 'u', 'v', 'w', 'x', 'y', 'z', '\0'};
  // int lens_V[2];
  // lens_V[0] = W[0].nrow;
...
void random_laplacian_tensor(Tensor<> &V, int N, int s, bool sparse_V,
  ...
  for (int k = 2; k <= d - 1; k++) {
    ...
    for (int jj = 0; jj < 2 * (k - 1); jj++)
      seq_I1[jj] = 'a' + jj;
    for (int jj = 2 * (k - 1); jj < 2 * k; jj++)
      seq_D[jj - 2 * (k - 1)] = 'a' + jj;
    for (int jj = 2 * k; jj < 2 * d; jj++)
      seq_I2[jj - 2 * k] = 'a' + jj;
    // contract
...
void randomized_svd(Matrix<> &A, Matrix<> &U, Vector<> &s, Matrix<> &VT2, int r,
                    int iter) {
  int n = A.ncol;
  Matrix<> X = Matrix<>(n, r);
  X.fill_random(0, 1);
...
/** Compute the rank 1 update vector on A(n).
    A*gamma = M
    */
void get_rankR_update_cholesky(int R, Matrix<> &xU, Vector<> &xS, Matrix<> &xVT,
...
void get_rankR_update_svd(int R, Matrix<> &xU, Vector<> &xS, Matrix<> &xVT,
                          Matrix<> &M, Matrix<> &A, Matrix<> &gamma,
                          bool random) {
  ...
  // S = 1./S**.5
  Transform<> sqrtinv([](double &d) { d = 1. / sqrt(d); });
  sqrtinv(S["i"]);
  // X = RHS @ U
...
void apply_rankR_update(Matrix<> &U, Vector<> &sigma, Matrix<> &VT, Matrix<> &A,
  ...
  U["ij"] = U["ij"] * sigma["j"];
  (*cached_tensor)[seq2] =
      (*cached_tensor)[seq2] + VT[seq_VT] * U[seq_U] * V[seq];
  A["ij"] = U["ik"] * VT["kj"];
}
...
void fold_unfold(Tensor<> &X, Tensor<> &Y) {
  ...
  double *vals_X;
  int64_t n_X;
  // if global index ordering is preserved between the two tensors, we can fold
  // simply
